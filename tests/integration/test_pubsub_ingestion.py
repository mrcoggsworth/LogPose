...
def test_pubsub_message_becomes_alert_in_rabbitmq(pubsub_clients, rabbitmq_channel) -> None:
    ...
    publisher_client.publish(topic_path, data=json.dumps(test_payload).encode("utf-8")).result()
    ...
    assert any(
        q["source"] == "pubsub" for q in queued
