"""Canonical, provider-aware identity extracted from alert payloads.

A ``Principal`` is the actor referenced by an alert (an AWS IAM role, a GCP
service account, an Active Directory user). It is *not* a full identity
record — just enough that two alerts referencing the same actor produce the
same ``cache_key()``, and enough to populate the ``principal.user`` noun of
a UDM event via ``to_udm_user()``.

Formerly ``logpose.enrichers.principal``; moved here when enrichment moved
into N8N workflows and identity normalization moved into the UDM mappers.
"""
...
from logpose.models.udm import UdmUser
...
class Principal(BaseModel):
    ...
    def to_udm_user(self) -> UdmUser:
        """Convert to the user portion of a UDM noun."""
        emails = [self.raw_id] if "@" in self.raw_id else []
        return UdmUser(
            userid=self.normalized_id,
