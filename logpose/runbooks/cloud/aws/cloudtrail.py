...
The pod parses six basic fields off ``alert.raw_payload`` (preserved
from the original Phase II implementation) and runs four CloudTrail
enrichers to attach principal identity, recent history, successful
writes, and inspected resources.
...
class CloudTrailRunbook(BaseRunbook):
    ...
    Parses six basic fields from the alert (legacy behaviour) and runs
    the four CloudTrail enrichers to augment ``EnrichedAlert.extracted``
    with ``cloudtrail`` / ``principal`` / ``enricher_errors`` keys.
    ...
    # by some tests that skip ``__init__`` to test ``_extract_basic_fields``
    # in isolation) still expose attributes ``enrich()`` reads.
    ...
    def __init__(
        ...
        *,
        ...
        logger.info(
            "CloudTrailRunbook started (total_budget=%.1fs).",
            budget,
        )
    ...
    def enrich(self, alert: Alert) -> EnrichedAlert:
        ...
        try:
            ...
            if self._pipeline is not None:
                ...
                # extracted is mutated in place by the pipeline; promote
                # the principal and any errors into reserved top-level keys.
            ...
            # Last-line defense — the pipeline shouldn't raise, but if
            # anything in the runbook does, no silent drop.
    ...
    def _extract_basic_fields(alert: Alert) -> dict[str, Any]:
        """Six basic fields parsed directly from the alert payload."""
