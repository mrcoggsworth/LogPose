...
`EnrichedAlert` is the output type of every workflow worker: it is built from the N8N workflow's HTTP response. It wraps the original `Alert` alongside the workflow's extracted fields, a timestamp, and an optional error string. Once published to the `enriched` queue it is the handoff point to Phase IV logging destinations.

```
Alert (raw_payload)
      │
      ▼
WorkflowWorker._build_enriched()
      │
      └── EnrichedAlert
            ├── alert          — original Alert, unchanged
            ├── workflow       — dot-path name, e.g. "cloud.aws.cloudtrail"
            ├── enriched_at    — UTC datetime, set automatically
            ├── extracted      — dict of workflow-returned fields (default: {})
            └── workflow_error  — None, or an error string if enrich() caught an exception
```
...
| `workflow` | `str` | required | Dot-separated route/workflow path |
| `enriched_at` | `datetime` | `datetime.now(tz=utc)` | Always UTC |
| `extracted` | `dict[str, Any]` | `{}` | Workflow-returned fields; empty on error |
| `workflow_error` | `str \| None` | `None` | Set when `enrich()` caught an exception |
...
| `test_enriched_alert_workflow_error_defaults_to_none` | `workflow_error` defaults to `None` when not passed |
| `test_enriched_alert_serializes_to_json` | `model_dump_json()` produces valid JSON containing `workflow`, `extracted`, and nested `alert` fields |
| `test_enriched_alert_with_workflow_error` | `workflow_error` can be set to an error string and is preserved verbatim |

### Constructing an EnrichedAlert in tests

The typical construction pattern for worker unit tests — where you build an `EnrichedAlert` to assert against — follows this shape:
...
When a workflow reports a handled error in its response, the worker sets `workflow_error` on a partial `EnrichedAlert`. To assert that path is working:
...
`EnrichedAlert` is the message body in the `enriched` queue. To inspect a real one, run the pipeline end-to-end against the N8N mock (`docker/n8n_workflow_mock.py`):

1. Start Docker Compose
2. Publish a CloudTrail alert to `alerts`
3. Start the Router — it dispatches to `workflow.cloudtrail`
4. Start the CloudTrail workflow worker — its N8N workflow enriches and the worker publishes to `enriched`
...
enriched = EnrichedAlert(alert=alert, workflow="test")

try:
    enriched.workflow = "something.else"
...
| [`logpose/workflows/worker.py`](../../../logpose/workflows/worker.py) | WorkflowWorker — builds `EnrichedAlert` from the N8N response and publishes it to the `enriched` queue |
| [`logpose/queue/queues.py`](../../../logpose/queue/queues.py) | Queue name constants — `QUEUE_ENRICHED` |
| [`tests/unit/test_enriched_alert.py`](../../../tests/unit/test_enriched_alert.py) | Unit tests for the EnrichedAlert model |
| [`tests/unit/test_workflow_worker.py`](../../../tests/unit/test_workflow_worker.py) | Shows `EnrichedAlert` construction from workflow responses |
| [`tests/integration/test_routing_flow.py`](../../../tests/integration/test_routing_flow.py) | Integration test that validates a real `EnrichedAlert` from the `enriched` queue |
